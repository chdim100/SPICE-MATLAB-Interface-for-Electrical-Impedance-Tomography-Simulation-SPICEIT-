function thorax_model=Set_Thorax(path,state,freq,electrode_deviation,Rel,conductivity_values)
clc
%%%% Creates 3D F.E.M. and SPICE Netgen Thorax models
%%%Inputs:
%%%-path: path to EIDORS folder (...\eidors-v3.9-ng\eidors\)
%%%-state: Inflated or Deflated
%%%-freq: Signal Frequency (15000 or 100000 Hz)
%%%-electrode_deviation: set electrode deviation. 'Symmetric' or
%%%'Assymetric'
%%%-Rel: '003' or '005' (normalized circular electrode radius)
%%%-conductivity_values: 'constant' for constant conductivity in each
%%%tissue, 'variational' for std conductivities
%%%

%%%Outputs: 
%%%-thorax_model: F.E.M. thorax 3D model extracted

%thorax_model=Set_Thorax('Inflated',15000,'Symmetric',0.05,'variational');
...
%%%%set geometry
initial_struct=['Thorax_' state '_' electrode_deviation '_Rel=' num2str(Rel)];
...
if exist(structure_geometry,'file')~=2
    [img,elec_pos]=create_thorax_geometry(structure_geometry,state,electrode_deviation,Rel,path);
else
...
name=['Thorax2020_' state '_' electrode_deviation '_' num2str(round(freq/1000)) 'k'];
...
llunginf=llung;
llunginf(2:8,2)=llunginf(2:8,2)+0.14;
% one could also run:
